...
| `DASHBOARD_EVENTS` | Push events / attach chips to the latest-events widget |
...
3. **Keep `manifest.json` valid** — `id` format `ORG/REPO` (recommended for publishable addons) or `REPO` (local/official-style only — not publishable; StreamKit+ may warn on local install), `app_version` must match the minimum StreamKit+ version you target.
